...
namespace Rapicgen.Tests.Generators
{
    public class CodeGeneratorFactoryTests
    {
        ...
        public void Can_Create_NSwagCodeGenerator()
            => sut.Create(string.Empty, string.Empty, string.Empty, SupportedLanguage.CSharp, SupportedCodeGenerator.NSwag)
                .Should()
                .BeOfType<NSwagCSharpCodeGenerator>();

        [Xunit.Fact]
        public void Can_Create_SwaggerCodeGenerator()
            => sut.Create(string.Empty, string.Empty, string.Empty, SupportedLanguage.CSharp, SupportedCodeGenerator.Swagger)
                .Should()
                .BeOfType<SwaggerCSharpCodeGenerator>();

        [Xunit.Fact]
        public void Can_Create_OpenApiCodeGenerator()
            => sut.Create(string.Empty, string.Empty, string.Empty, SupportedLanguage.CSharp, SupportedCodeGenerator.OpenApi)
                .Should()
                .BeOfType<OpenApiCSharpCodeGenerator>();

        [Xunit.Fact]
        public void Can_Create_RefitterCodeGenerator()
            => sut.Create(string.Empty, string.Empty, string.Empty, SupportedLanguage.CSharp, SupportedCodeGenerator.Refitter)
                .Should()
                .BeOfType<RefitterCodeGenerator>();

        [Xunit.Fact]
        public void Create_NSwagStudio_Throws_NotSupported()
            => new Action(() => sut.Create(string.Empty, string.Empty, string.Empty, SupportedLanguage.CSharp, SupportedCodeGenerator.NSwagStudio))
                .Should()
