...
class GenerateRefitterAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val outputFilePath = if (outputDirectory.isBlank()) {
            File(specificationFilePath).parent + File.separator + "RefitterOutput.cs"
        } else {
            project.basePath + File.separator + outputDirectory + File.separator + "RefitterOutput.cs"
        }
        ...
        val command = "rapicgen csharp refitter "$specificationFilePath" "$namespace" "$outputFilePath""
        executeRapicgenCommand(project, command, "Refitter")
    }
