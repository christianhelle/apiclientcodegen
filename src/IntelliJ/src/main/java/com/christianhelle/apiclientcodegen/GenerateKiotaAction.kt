...
class GenerateKiotaAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val outputFilePath = if (outputDirectory.isBlank()) {
            File(specificationFilePath).parent + File.separator + "KiotaOutput.cs"
        } else {
            project.basePath + File.separator + outputDirectory + File.separator + "KiotaOutput.cs"
        }
        ...
        val command = "rapicgen csharp kiota "$specificationFilePath" "$namespace" "$outputFilePath""
        executeRapicgenCommand(project, command, "Microsoft Kiota")
    }
