...
class GenerateAutoRestAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val outputFilePath = if (outputDirectory.isBlank()) {
            File(specificationFilePath).parent + File.separator + "AutoRestOutput.cs"
        } else {
            project.basePath + File.separator + outputDirectory + File.separator + "AutoRestOutput.cs"
        }
        ...
        val command = "rapicgen csharp autorest "$specificationFilePath" "$namespace" "$outputFilePath""
        executeRapicgenCommand(project, command, "AutoRest")
    }
