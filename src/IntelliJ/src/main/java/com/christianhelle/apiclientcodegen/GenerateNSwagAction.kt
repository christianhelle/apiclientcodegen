...
class GenerateNSwagAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val outputFilePath = if (outputDirectory.isBlank()) {
            File(specificationFilePath).parent + File.separator + "NSwagOutput.cs"
        } else {
            project.basePath + File.separator + outputDirectory + File.separator + "NSwagOutput.cs"
        }
        ...
        val command = "rapicgen csharp nswag "$specificationFilePath" "$namespace" "$outputFilePath""
        executeRapicgenCommand(project, command, "NSwag")
    }
