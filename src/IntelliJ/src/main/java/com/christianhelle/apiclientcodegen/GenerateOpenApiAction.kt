...
class GenerateOpenApiAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val outputFilePath = if (outputDirectory.isBlank()) {
            File(specificationFilePath).parent + File.separator + "OpenApiOutput.cs"
        } else {
            project.basePath + File.separator + outputDirectory + File.separator + "OpenApiOutput.cs"
        }
        ...
        val command = "rapicgen csharp openapi "$specificationFilePath" "$namespace" "$outputFilePath""
        executeRapicgenCommand(project, command, "OpenAPI Generator")
    }
